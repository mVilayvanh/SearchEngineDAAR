...
public class RegExParser {
  //MACROS
    ...
    static final int PLUS = 0xE09;

  //REGEX
    private final String regEx;
  
  //CONSTRUCTOR
    public RegExParser(InputStream in){
        Objects.requireNonNull(in);
        var sc = new Scanner(in);
        regEx = sc.next();
        if (regEx.isEmpty()) throw new IllegalArgumentException("Must have at least one character");
    }
  ...
  //FROM REGEX TO SYNTAX TREE
    public RegExTree parse() throws Exception {
	    ...
	    for (int i=0;i<regEx.length();i++) {
		    ...
		    result.add(new RegExTree(charToRoot(current),new ArrayList<RegExTree>()));
	    }
    ...
    // Priority must be set here for operator
    // It is descending priority
    private static RegExTree processToken(ArrayList<RegExTree> result) throws Exception {
    ...
    private static boolean containParenthese(ArrayList<RegExTree> trees) {
        return trees.stream().anyMatch(t -> t.root==PARENTHESEFERMANT || t.root==PARENTHESEOUVRANT);
    }
    private static ArrayList<RegExTree> processParenthese(ArrayList<RegExTree> trees) throws Exception {
        ...
        for (RegExTree t: trees) {
            if (!found && t.root==PARENTHESEFERMANT) {
                ...
                subTrees.add(processToken(content));
    ...
    private static boolean containEtoile(ArrayList<RegExTree> trees) {
        return trees.stream().anyMatch(t -> t.root==ETOILE && t.subTrees.isEmpty());
    }
    ...
    private static boolean containPlus(ArrayList<RegExTree> trees) {
        return trees.stream().anyMatch(t -> t.root==PLUS && t.subTrees.isEmpty());
    }
    ...
    private static boolean containAltern(ArrayList<RegExTree> trees) {
        return trees.stream().anyMatch(t -> t.root==ALTERN && t.subTrees.isEmpty());
    }
